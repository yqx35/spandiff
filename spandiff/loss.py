...
from modules.loss_o import SelfAdjDiceLoss
from modules.utils import entity_iou
class Loss(ABC):
    def compute(self, *args, **kwargs):
        pass

class SpanDiffLoss(Loss):
    def __init__(self, entity_type_count, device, model, optimizer, scheduler, max_grad_norm, nil_weight, match_class_weight, match_boundary_weight, loss_class_weight, loss_boundary_weight, match_boundary_type, type_loss, solver):
        ...
        losses = ['labels', 'boundary','vfl']
        self.weight_dict = {'loss_ce': loss_class_weight, 'loss_boundary': loss_boundary_weight,'loss_vfl':5}
...
class Criterion(nn.Module):
    ...
    def __init__(self, entity_type_count, weight_dict, nil_weight, losses, type_loss, match_class_weight, match_boundary_weight, match_boundary_type, solver):
        ...
        self.sadice = SelfAdjDiceLoss(alpha=ALPHA, gamma=GAMMA)
        self.alpha = 0.2
        self.gamma = 2
    ...
    def loss_labels(self, outputs, targets, indices, num_spans):
        ...
        if self.type_loss == "bceloss":
            ...
            losses = {'loss_ce': loss_ce.mean()}
        if self.type_loss =="sadice":
            # print('------')
            src_logits = src_logits.view(-1, src_logits.size(2))
            target_classes = target_classes.view(-1)
            loss_ce = self.sadice(src_logits,target_classes)
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        return losses
    def loss_labels_vfl(self,outputs, targets, indices, num_spans):
        idx = self._get_src_permutation_idx(indices)
        src_spans_left = outputs['pred_left'][idx]
        src_spans_right = outputs['pred_right'][idx]
        pred_spans_left_p = src_spans_left.argmax(dim=-1)
        pred_spans_right_p = src_spans_right.argmax(dim=-1)
        ...
        pred_spans  = torch.stack([pred_spans_left_p, pred_spans_right_p], dim=-1)
        targets_spans = torch.stack([target_spans_left, target_spans_right], dim=-1)
        ious, _ = entity_iou(pred_spans,targets_spans)
        ious = torch.diag(ious).detach()

        src_logits = outputs['pred_logits']

        # print('00:',src_logits.shape)
        ...
        target_classes[idx] = target_classes_o
        target = F.one_hot(target_classes, num_classes=self.entity_type_count)#[..., :-1]
        ...
        # loss = loss.mean(1).sum() * src_logits.shape[1] / num_spans
