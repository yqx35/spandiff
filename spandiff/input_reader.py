...
from spandiff.entities import Dataset, EntityType, Entity, Document, DistributedIterableDataset
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_document(self, doc, dataset: Dataset) -> Document:
        ...
        if len(doc_encoding) > 512:
            self._log(f"Document  len(doc_encoding) = {len(doc_encoding) } > 512, Ignored!")
            return None
