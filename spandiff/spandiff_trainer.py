...
from spandiff import models
from spandiff import sampling
from spandiff import util
from spandiff.entities import Dataset
from spandiff.evaluator import Evaluator
from spandiff.input_reader import JsonInputReader, BaseInputReader
from spandiff.loss import SpanDiffLoss, Loss
from tqdm import tqdm
from spandiff.trainer import BaseTrainer
...
class SpanDiffTrainer(BaseTrainer):
    ...
    def load_model(self, input_reader, is_eval = False):
        ...
        total_params = util.count_parameters(model, only_trainable=False)
        trainable_params = util.count_parameters(model, only_trainable=True)
        self._logger.info(f"Model parameters: Total {util.format_num_params(total_params)}, Trainable {util.format_num_params(trainable_params)}")
        return model
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        compute_loss = SpanDiffLoss(input_reader.entity_type_count, self._device, model, optimizer, scheduler, args.max_grad_norm, args.nil_weight, args.match_class_weight, args.match_boundary_weight, args.loss_class_weight, args.loss_boundary_weight, args.match_boundary_type, args.type_loss, solver = args.match_solver)
